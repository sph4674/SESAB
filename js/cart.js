...
function displayCartItems(currentCart) {
  ...
  currentPrice = [];
  let cartCount = 0; // Variable to keep track of cart item count
  ...
  currentCart.forEach((item) => {
    ...
    // Increment the cart item count for each item in the cart
    cartCount += item.qty;
  });

  totalPrice.innerText = currentPrice.reduce((curr, acc) => curr + acc, 0).toFixed(2);
  cartIconTotal.innerText = cartCount; // Update the cart item count in the header
...
function handleDelete(e) {
  ...
  let currentCart = getLocalStorage('currentCart');
  let currentQty = getLocalStorage('qtyCart');

  const deletedItemIndex = currentCart.findIndex((product) => product.id === id);
  const deletedItem = currentCart[deletedItemIndex];
  const deletedQty = deletedItem.qty;

  if (deletedQty > 1) {
    // If quantity is more than 1, decrement the quantity of the deleted item
    currentCart[deletedItemIndex].qty -= 1;
    currentQty -= 1;
  } else {
    // If quantity is 1, remove the item from the cart
    currentCart = currentCart.filter((product) => product.id !== id);
    currentQty -= 1;
  }
  ...
  if (currentCart.length === 0) {
    // Clear the cart count and display an empty cart message
    cartIconTotal.innerText = '0';
    currentCartItemsContainer.innerHTML = '<p>Your cart is empty.</p>';
    totalPrice.innerText = '0.00';
  } else {
    setLocalStorage(cartIconTotal.innertext,'0')
...
export { displayCartItems, selectElementsForCart, clearCart };
